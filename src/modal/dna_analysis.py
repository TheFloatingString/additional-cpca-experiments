...
This script:
1. Loads the DNA dataset from OpenML (id=40670)
...
- 80% train, 20% split
- Dataset: DNA from OpenML (id=40670)
- Class split: 1 class as background, 2 classes as foreground
"""
...
def run_dna_analysis():
    ...
    print("Loading DNA dataset from OpenML (id=40670)...")
    dataset = openml.datasets.get_dataset(40670)

    # Sanity check: verify this is the DNA dataset
    ...
    # Split classes: 1 class as background, 2 classes as foreground
    unique_classes = np.unique(y)
    shuffled_classes = unique_classes.copy()
    np.random.shuffle(shuffled_classes)

    # 1 class for background, remaining 2 for foreground
    background_classes = shuffled_classes[:1]
    foreground_classes = shuffled_classes[1:]

    print(f"\nForeground classes (2 classes): {sorted(foreground_classes.tolist())}")
    print(f"Background classes (1 class): {sorted(background_classes.tolist())}")
